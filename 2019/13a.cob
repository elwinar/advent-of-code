       ...
       Begin.
           ...
           PERFORM VARYING Ptr FROM 1 BY 3 UNTIL Ptr > OutputsLen
            MOVE Outputs(Ptr) TO PosX
            MOVE Outputs(Ptr + 1) TO PosY
            MOVE Outputs(Ptr + 2) TO Tile(PosX, PosY)
           END-PERFORM.

           MOVE 0 TO BlocksNumber.
           PERFORM VARYING PosX FROM 1 UNTIL PosX > 99
                     AFTER PosY FROM 1 UNTIL PosY > 99
            IF Tile(PosX, PosY) IS EQUAL TO 2 
             ADD 1 TO BlocksNumber
            END-IF
           END-PERFORM.
           DISPLAY BlocksNumber.
